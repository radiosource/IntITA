...
class Tests extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('fkModuleID, fkLectureID, TestName, TestDescription, TestURL', 'required'),
			array('TestID, fkModuleID, fkLectureID', 'numerical', 'integerOnly'=>true),
			array('TestName, TestDescription, TestURL', 'length', 'max'=>45),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('TestID, fkModuleID, fkLectureID, TestName, TestDescription, TestURL', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('TestDescription',$this->TestDescription,true);
		$criteria->compare('TestURL',$this->TestURL,true);
