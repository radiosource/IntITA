...
class Videos extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('fkModuleID, fkLectureID, VideoName, VideoDescription, VideoURL, VideoDuration', 'required'),
			array('VideoID, fkModuleID, fkLectureID, VideoDuration', 'numerical', 'integerOnly'=>true),
			array('VideoName, VideoDescription, VideoURL', 'length', 'max'=>45),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('VideoID, fkModuleID, fkLectureID, VideoName, VideoDescription, VideoURL, VideoDuration', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('VideoDuration',$this->VideoDuration);
